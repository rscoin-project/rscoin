...
The build script we'll be using is contained in the RSCOIN github repository ([contrib/gitian-build.py](https://github.com/rscoin-project/rscoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the RSCOIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/RSCOIN-Project/RSCOIN/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, rscoin-detached-sigs, and rscoin GitHub repos.
