...
- [Development guidelines](#development-guidelines)
    - [General RSCOIN Core](#general-rscoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
RSCOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
RSCOIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/rscoin-project/rscoin/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/rscoind -printtoconsole
```
...
However, in open source software such as RSCOIN Core, where everyone uses
...
pay attention to for reviewers of RSCOIN Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: RSCOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against RSCOIN Core so that they can be integrated
...
file descriptors it will cause problems with RSCOIN's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:RSCOIN-Project/RSCOIN.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `rscoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `rscoin-cli` and the GUI debug console use this table to determine how to
