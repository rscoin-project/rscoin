...
*Setup instructions for a gitian build of RSCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the RSCOIN
...
to the RSCOIN GitHub release page.
...
- [Building RSCOIN Core](#building-rscoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/rscoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/rscoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/rscoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/rscoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/rscoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/rscoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/rscoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@rscoins.io) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/rscoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/rscoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/rscoin-osx-*-build.assert
```
