...
/** Class encapsulating RSCOIN Core startup and shutdown.
...
/** Main RSCOIN application object */
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // RSCOIN: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", PIVXGUI::tr("A fatal error occurred. RSCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("RSCOIN Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("RSCOIN Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
